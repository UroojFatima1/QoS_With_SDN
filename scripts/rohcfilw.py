...
CONGESTED_TO_UNCON_TIME = 3  # If the congested links become non-congested, h2 returns back to original shortest path again.
MAX_BW = 1250000  # Maximum throughput capacity on the link
ENABLE_QoS = True  # Set QoS properties with boolean expression.
IS_CONGESTED_PERIOD = 1  # Congestion check period to check the each link on the h2 path.

#   Mininet mac and ip addresses of h2 and h3
...
class QoS(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.h2_is_running = False
        self.h2_to_h3 = []  # h2 to h3 traffic
        self.h3_to_h2 = []  # h3 to h2 traffic
        ...
        #   Another hub is created to check congestion link on the h2 path.
    ...
    def new_host_handler(self, ev):
        ...
        if (host.mac, host.port.dpid) not in self.graph.edges:
            self.graph.add_edge(host.mac, host.port.dpid,
                                **{'link_load': 0.0, 'time': 0.0, 'congested': False, 'non-congested_time': 0.0,
                                   'weight': 0.0,
                                   'old_num_bytes': 0, 'curr_speed': 1000000})
            self.graph.add_edge(host.port.dpid, host.mac,
                                **{'port': host.port.port_no, 'link_load': 0.0, 'time': 0.0, 'congested': False,
                                   'non-congested_time': 0.0, 'weight': 0.0,
                                   'old_num_bytes': 0, 'curr_speed': 1000000})
    ...
    def _is_congested(self):
        ...
        hub.sleep(5)
        while True:
            if self.h2_is_running and ENABLE_QoS:
                # calculate the number of hops on h2 path
                h2_path = self.h2_to_h3
                for link in range(len(h2_path) - 1):
                    if self.graph[h2_path[link]][h2_path[link + 1]]['congested']:  # check here again.
                        self.logger.info(
                            'Congested path(h2 to h3) between switch %s and %s' % (h2_path[link], h2_path[link + 1]))
                        self._delete_h2_path_flows(h2_path)
                h2_path = self.h3_to_h2
                for link in range(len(h2_path) - 1):
                    if self.graph[h2_path[link]][h2_path[link + 1]]['congested']:  # check here again.
                        self.logger.info(
                            'Congested path(h3 to h2) between switch %s and %s' % (h2_path[link], h2_path[link + 1]))
                        self._delete_h2_path_flows(h2_path)
            # Check congested links of h2 path periodically e.g. 0,5 sec
            hub.sleep(IS_CONGESTED_PERIOD)

    def _delete_h2_path_flows(self, path):
        """
        ...
        for datapath in (path):
            ...
            #   Delete entries for h2 to h3 path
            if path == self.h2_to_h3:
                #   IDEA: create matching using h2 mac and h3 destination mac.
                match = parser.OFPMatch(dl_src=h2_MAC, dl_dst=h3_MAC)
                req = parser.OFPFlowMod(datapath=dp, match=match, command=command, out_port=out_port)
                dp.send_msg(req)

            #   Delete entries for h3 to h2 entries
            else:
                match = parser.OFPMatch(dl_src=h3_MAC, dl_dst=h2_MAC)
                req = parser.OFPFlowMod(datapath=dp, match=match, command=command, out_port=out_port)
                dp.send_msg(req)

        self.logger.info("Deleted path %s " % (path))
        #   After the delete messages, clean the path from/to h2. and then stop checking(e.g False)
    ...
    def _print_topo(self):
        ...
        while True:
            ...
            if h2_MAC in self.graph.nodes:
                fixed_positions[h2_MAC] = (-1, 0)
                labels[h2_MAC] = 'H'
            if h3_MAC in self.graph.nodes:
                fixed_positions[h3_MAC] = (4, 0)
                labels[h3_MAC] = 'HC'
            if h1_MAC in self.graph.nodes:
                fixed_positions[h1_MAC] = (1.5, 1.5)
                labels[h1_MAC] = 'h1'

            fixed_nodes = fixed_positions.keys()

            if len(self.h3_to_h2) > len(self.h2_to_h3):
                for i in range(len(self.h3_to_h2) - 1):
                    h2_path_edges.append((self.h3_to_h2[i], self.h3_to_h2[i + 1]))
            else:
                for i in range(len(self.h2_to_h3) - 1):
                    h2_path_edges.append((self.h2_to_h3[i], self.h2_to_h3[i + 1]))
            ...
            nx.draw_networkx_edges(self.graph, pos, h2_path_edges, **options2)
                ...
                try:
                    if self.graph[dpid][edge_candidate]['port'] == stat.port_no:
                        ...
                        if self.graph[dpid][edge_candidate]['link_load'] > MAX_BW * 0.7:
                            ...
                            self.graph[dpid][edge_candidate]['congested_pre'] = self.graph[dpid][edge_candidate][
                                'congested']
                            ...
                            if self.graph[dpid][edge_candidate]['congested_pre'] != self.graph[dpid][edge_candidate][
                                'congested']:
                            ...
                            if self.graph[dpid][edge_candidate]['congested_to_uncon']:
                                ...
                                if self.graph[dpid][edge_candidate][
                                    'non-congested_time'] == CONGESTED_TO_UNCON_TIME and not self.con_to_noncon_flag and ENABLE_QoS:
                                    if len(self.h3_to_h2) > 0:
                                        self._delete_h2_path_flows(self.h3_to_h2)
                                    else:
                                        self._delete_h2_path_flows(self.h2_to_h3)
                                    self.con_to_noncon_flag = True
                                elif self.graph[dpid][edge_candidate][
                                    'non-congested_time'] == CONGESTED_TO_UNCON_TIME - 1 and ENABLE_QoS:
    ...
    def calculate_path_to_server(self, src, dst, balanced):
        ...
        """
        path_out = []
        #   Use QoS path for both h2 and h3
        if balanced:
            #   Load balanced routing for h2
    ...
    def _handle_ipv4(self, datapath, in_port, pkt):
        ...
        if (net_src == h2_IP and net_dst == h3_IP) or (net_src == h3_IP and net_dst == h2_IP):
            #   Calculate QoS path the h2.
            routing_path = self.calculate_path_to_server(
                datapath.id, self.ip_to_mac.get(net_dst, eth_dst_in), balanced=True)
            #   If the flow starts from h2 to h3.
            if net_src == h2_IP and net_dst == h3_IP:
                for hop in routing_path:
                    if hop['dpid'] not in self.h2_to_h3:
                        self.h2_to_h3.append(hop['dpid'])
                self.h2_is_running = True
            #   If the flow starts from h3 to h2.
            if net_src == h3_IP and net_dst == h2_IP:
                for hop in routing_path:
                    if hop['dpid'] not in self.h3_to_h2:
                        self.h3_to_h2.append(hop['dpid'])
                self.h2_is_running = True
            self.logger.info(
                "h2 Calculated path from %s-%s: %s" % (datapath.id, self.ip_to_mac.get(net_dst, eth_dst_in),
                                                       routing_path))
        else:
            ...
            self.logger.info(
                "OTHER Calculated path from %s-%s: %s" % (datapath.id, self.ip_to_mac.get(net_dst, eth_dst_in),
                                                          routing_path))
    ...
    def _packet_in_handler(self, ev):
        ...
        if arp_header:  # we got an ARP
            ...
            hc=RohcCompressor(cid_type=ROHC_LARGE_CID, profiles=[ROHC_PROFILE_RTP], verbose=TRUE)
            if hc is None:
                print("failed to create the ROHC compressor")
                sys.exit(1)
            (status,pkt2) = hc.compress(pkt)
            ...
            hc.clear()
            hc.reset()
            self._handle_ipv4(datapath, in_port, pkt2)
        elif ipv6_header:
            return
        else:
