...
IS_CONGESTED_PERIOD = 2.5                   #   Congestion check period to check the each link on the h2 path.
...
class QoS(app_manager.RyuApp):
            ...
            if self.h2_is_running and ENABLE_QoS:
                ...
                for link in range(len(h2_path)-1):
                    if self.graph[h2_path[link]][h2_path[link+1]]['congested']: #check here again.
                        self.logger.info('Congested path(h2 to h3) between switch %s and %s' % (h2_path[link], h2_path[link+1]))
                        self._delete_h2_path_flows(h2_path)
                h2_path = self.h3_to_h2
                for link in range(len(h2_path)-1):
                    if self.graph[h2_path[link]][h2_path[link+1]]['congested']: #check here again.
                        self.logger.info('Congested path(h3 to h2) between switch %s and %s' % (h2_path[link], h2_path[link+1]))
                        self._delete_h2_path_flows(h2_path)
    ...
    def _packet_in_handler(self, ev):
        """
        Is called if a packet is forwarded to the h3. Packet handling is done here.
